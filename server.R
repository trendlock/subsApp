...
shinyServer(function(input, output, session) {
  ...
  observeEvent(drawChart(), {
      ...
      if(input$tog_input == "Input Jet Effieciency") {
        ...
        #write_rds(drawn_vals, glue::glue("saved_jet_input_{now() %>% as.numeric}.rds"))
  ...
  observeEvent(input$replot_all,{
    ...
    df <- tibble(kts = seq(0.5, 20, 0.5),
                 eff.jet = read_rds("saved_jet_input.rds"),
                 eff.prop = read_rds("saved_prop_input.rds")
    )
